...
instance Monad f => Bind (StateT s f) where
  (=<<) :: (a -> StateT s f b)
        -> StateT s f a
        -> StateT s f b
  f =<< StateT a = StateT $ ((=<<) (shiftSt f)) . a
    where shiftSt f' (a', t) = runStateT (f' a') t
