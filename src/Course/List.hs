...
filter :: (a -> Bool)
       -> List a
       -> List a
filter criterion = foldRight select Nil
  where select x | criterion x = (x:.)
                 | otherwise   = id
...
find :: (a -> Bool)
     -> List a
     -> Optional a
find criterion list = case filter criterion list of
                        Nil      -> Empty
                        (x :. _) -> Full x
...
produce :: (a -> a)
        -> a
        -> List a
produce func seed = seed :. produce func (func seed)
...
permutations xs0 = xs0 :. perms xs0 Nil
  where perms Nil      _  = Nil
        perms (t:. ts) is = foldRight (\xs -> snd . interleave' id xs) (perms ts (t:.is)) (permutations is)
          where interleave' _ Nil     r = (ts, r)
